...
func LoadConfigurationFromFile(configFile string) (*Config, error) {
	var c Config
...
func InitializeShardTopology(shards []Shard, currShardName string) (*Shards, error) {
...
func (s *Shards) DetermineTargetShard(key string) int {
	h := fnv.New64()
	h.Write([]byte(key))
