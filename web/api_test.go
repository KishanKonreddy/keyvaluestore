...
func createShardDb(t *testing.T, idx int) *db.Database {
	...
	d, close, err := db.CreateDatabaseInstance(fileName)
	if err != nil {
...
func TestWebServer(t *testing.T) {
	...
	ts1 := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if strings.HasPrefix(r.RequestURI, "/retrieve") {
			shard1Get(w, r)
		} else if strings.HasPrefix(r.RequestURI, "/store") {
			shard1Set(w, r)
		}
	}))
	defer ts1.Close()
	...
	shard1Get = web1.HandleValueRetrieval
	shard1Set = web1.HandleValueStorage
	shard2Get = web2.HandleValueRetrieval
	shard2Set = web2.HandleValueStorage

	for key := range keys {
		_, err := http.Get(fmt.Sprintf("%s/store?key=%s&value=value-%s", ts1.URL, key, key))
		if err != nil {
	...
	for key := range keys {
		resp, err := http.Get(fmt.Sprintf("%s/retrieve?key=%s", ts1.URL, key))
		if err != nil {
